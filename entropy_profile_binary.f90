! S. Sullivan, Columbia, Nov 2018
module entropy_profile_binary
   use netcdf
...
CONTAINS
   ...
   subroutine entropy_run(input_file, varname, timepts, nx, ny)
      ...
      call check(nf90_inq_varid(id_nc, varname, varid),"Obtaining input varid")
           ! CLDC varid for Sara's output; LHFL_S for Pierre's output
      ...
      do i = 1, timepts
         ...
         do j = 2, MIN(nx,ny)
            ...
            H_vals(i,j) = H      
                 ! For Pierre's data H_vals(i-199,j) = H
            deallocate(out_mat)
         enddo
      enddo
      
      ! save the entropy values to an external file
      open(unit = 7, file = "entropy_mcs_monthly_grids",action="write",status="replace")
      do i = 1,timepts
         do j = 1,nx
      ...
      do i = 1, nx - window_size
         do j = 1, ny - window_size
            do i_offset = 1, window_size
               ...
               enddo
            enddo
            call same_val_prob(sub_mat, window_size, window_size, sum_val)
   ...
   ! equivalent of the entropy.m probability constructions
   ! calculates the probability (output_val) of the same value throughout input_mat 
   ! as in its upper left corner (output_val) in input_mat
   subroutine same_val_prob(input_mat, nx, ny, output_val)
      ...
      do i = 1, nx
         do j = 1, ny
            if (i == 1 .and. j == 1) then
               ...
               if (input_mat(i,j) == num) pr = pr + 1d0   ! simple equality
               !if (num == 0. .and. input_mat(i,j) == num) pr = pr + 1d0  ! equality only if no MCS
               !if (num > 0. .and. input_mat(i,j) > 0.) pr = pr + 1d0     ! equality only if at least one MCS
            endif
         enddo
      enddo

      output_val = REAL(pr / (nx * ny))

   end subroutine same_val_prob
...
end module entropy_profile_binary
